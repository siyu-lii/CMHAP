...
import numpy as np
from model.spatem_att import Model as Spatem


class CMModel(Module):
    def __init__(self, in_features=66, kernel_size=10, d_model=512, num_stage=2, dct_n=10,len_parts=2,
                 num_classes=15, num_joints=22,num_frames=40, num_heads=3, num_persons=1, num_channels=3,
                 kernel_size1=[3,5],config=None):
        ...
        config = [[64, 64, 16], [64, 64, 16],[64, 128, 32], [128, 128, 32],
            [128, 256, 64], [256, 256, 64],[256, 256, 64], [256, 256, 64]]
        self.predictlabl =Spatem(len_parts, num_classes, num_joints,
                 num_frames, num_heads, num_persons, num_channels,
                 kernel_size1, use_pes=True, config=config,
...
if __name__=='__main__':
    net_pred = CMModel(in_features=66, kernel_size=10, d_model=256,num_stage=12, dct_n=20,len_parts=2,
                        num_classes=15, num_joints=22, num_frames=40, num_heads=3, num_persons=1, num_channels=3,
                        kernel_size1=[3, 5]).cuda()
    history_poses=torch.randn([64,60,66]).cuda()

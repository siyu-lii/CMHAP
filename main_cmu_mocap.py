...
def main(opt):
    ...
    net_pred = Model.CMModel(in_features=in_features, kernel_size=kernel_size, d_model=d_model,
                        ...
                        num_classes=15, num_joints=25, num_frames=40, num_heads=3, num_persons=1, num_channels=3,
                        kernel_size1=[3, 5])
    net_pred.cuda()
...
def run_model(net_pred, optimizer=None, is_train=0, data_loader=None, epo=1, opt=None):
    ...
    joint_equal = np.array([15, 15, 15, 23, 23, 32, 32])  #需填充的目标关节索引
    ...
    for i, (label, p3d_h36) in tqdm(enumerate(data_loader)):
        ...
        if is_train == 0:
            ...
            loss_all = loss_p3d+opt.alpha*loss_classfied+opt.beta*loss_multi_scale

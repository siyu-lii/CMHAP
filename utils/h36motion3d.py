...
class Datasets(Dataset):
    def __init__(self, opt, actions=None, split=0):
        ...
        joint_name = ["Hips", "RightUpLeg", "RightLeg", "RightFoot", "RightToeBase", "Site", "LeftUpLeg", "LeftLeg",
                      ...
                      "RightForeArm", "RightHand", "RightHandThumb", "Site", "R_Wrist_End", "Site"]  #32个关节
        subs = subs[split]
        key = 0
            ...
            for action_idx in np.arange(len(acts)):
                ...
                if self.split <= 1:
                    ...
                    for subact in [1, 2]:  # subactions
                        ...
                        filename = '{0}/S{1}/{2}_{3}.txt'.format(self.path_to_data, subj, action, subact)  #./datasets/h3.6m/s1/direction_1
                    ...
                    #下采样（每2帧取1帧，减少计算量）
                    even_list = range(0, n, self.sample_rate)
                    num_frames1 = len(even_list)  # 下采样后的帧数
                    the_sequence1 = np.array(the_sequence1[even_list, :])  ## 下采样后的序列
                    the_seq1 = torch.from_numpy(the_sequence1).float().cuda(self.cuda_id)
                    the_seq1[:, 0:6] = 0
                    p3d1 = data_utils.expmap2xyz_torch(the_seq1)
                    # labels = np.zeros(len(acts),np.float32)
                    # labels[acts.index(action)] = 1
                    label_index=acts_sum.index(action)  #绑定动作类别标签（索引形式）,动作在列表中的索引（0-14
                    # print(label_index)
                    ...
                    #测试集不使用skip_rate，而是固定从 0 到（总帧数 - 100）截取样本，确保测试样本的一致性
                    fs_sel1 = np.arange(0, num_frames1 - 100)  # 测试集有效起始帧
                    valid_frames_1=fs_sel1
                    # 生成索引：(序列ID, 起始帧)，表示从该序列的起始帧开始截取seq_len帧
                    tmp_data_idx_1 = [key] * len(valid_frames_1) # 序列ID列表
                    tmp_data_idx_2 = list(valid_frames_1) # 起始帧列表
                    self.data_idx.extend(zip(tmp_data_idx_1, tmp_data_idx_2))  # 加入全局索引列表
                    num = num + len(tmp_data_idx_2)
        ...
        clas_number[-1] = np.sum(clas_number[:-1], axis=0)  #所有受试者的总样本数
        # ignore constant joints and joints at same position with other joints
        joint_to_ignore = np.array([0, 1, 6, 11, 16, 20, 23, 24, 28, 31])  #忽略10个关节

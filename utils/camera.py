...
from utils.utils_ktp import wrap
...
def world2camera(pt, Rw2c, Tw2c):
        """

        :return:
        """
        if isinstance(pt, np.ndarray):
            return pt @ Rw2c.T + Tw2c.T
        else:
            return pt @ torch.from_numpy(Rw2c.T).float().to(pt.device) + torch.from_numpy(Tw2c.T).float().to(pt.device)

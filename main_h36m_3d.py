...
def main(opt):
    ...
    net_pred = Model.CMModel(in_features=in_features, kernel_size=kernel_size, d_model=d_model,
                                 num_stage=opt.num_stage, dct_n=opt.dct_n,len_parts=2,
                        num_classes=15, num_joints=22, num_frames=40, num_heads=3, num_persons=1, num_channels=3,
                        kernel_size1=[3, 5])
    net_pred.cuda()
...
def L2MultiScaleLoss_train(out, gt,down_key_transform):
    ...
    # down_key_transform: downsample stragy: 22→10→6→4
    # this is index is adopted form https://github.com/cobblestones/CGHMP
    '''
    ...
    gt_p10=(gt_all_scales_down['p10']).permute(0,2,1)
    out_p10=(out_all_scales_down['p10']).permute(0,2,1)
    loss_p10=torch.mean(torch.norm(gt_p10.view(batch_size,seq_len,-1,3) - out_p10.view(batch_size,seq_len,-1,3), 2, dim=-1))

    gt_p6=(gt_all_scales_down['p6']).permute(0,2,1)
    out_p6=(out_all_scales_down['p6']).permute(0,2,1)
    loss_p6=torch.mean(torch.norm(gt_p6.view(batch_size,seq_len,-1,3) - out_p6.view(batch_size,seq_len,-1,3), 2, dim=-1))
...
def run_model(net_pred, optimizer=None, is_train=0, data_loader=None, epo=1, opt=None):
    ...
    down_key = [
        ...
        ('p10', 'p6', [[0, 1], [2, 3], [4, 5], [6, 7], [7, 8, 9], [10, 11, 12]]),
        ('p6', 'p4', [[0, 2], [1, 2], [3, 4], [5, 6]])]
    ...
    for i, (label, p3d_h36) in tqdm(enumerate(data_loader)):
        ...
        if is_train == 0:
            ...
            loss_all = loss_p3d + opt.alpha * loss_label_guided + opt.beta * loss_multi_scale_loss
